...
/* Return a node for the bt and initialize its members */
static struct node *
get_bt_node (void *data)
{
...
insert (struct node **head, void *data)
{
	...
	if (input <= value)
		prev->left = get_bt_node(data);
	else
		prev->right = get_bt_node(data);
...
delete_node (struct node **head, struct node *parent, int key)
{
	...
	while (ptr) {
        ...
        if (key == value) {
		printf("\nNode with the given element found. Deleting this node and adjusting the bt\n");
